            ...
            <ul>
                @if($categories)
                    @foreach($categories as $category)
                        <li class="unstyled">
                            <i class="glyphicon glyphicon-tag"></i><a href="{{URL::action('CategoriesController@getPosts',$category->id)}}"> {{ $category->name }}</a>
                        </li>
                    @endforeach
                @endif
            </ul>
        ...
        <div class="col-md-12" style="padding: 20px; border:1px solid white ; margin-bottom: 20px; background-color: #ececec !important;">
            ...
            <div class="col-md-8">
                <div class="row">
                    <div class="col-md-12">
                        <h4>
                            <strong><a href="{{action('BlogsController@show',$post->id) }}">{{ $post->title }}</a></strong>
                        </h4>
                    </div>
                </div>
                <div class="row">
